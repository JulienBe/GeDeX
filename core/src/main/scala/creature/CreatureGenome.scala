...
/**
  ...
  */
case class CreatureGenome(bodies: List[(BodyGenome, ShapeGenome)], val joints: List[JointGenome])
...
object CreatureGenome {
  ...
  def createGenome(minBodies: Int, maxBodies: Int) = {
    ...
    val nbJoint = Creator.valueInBounds(nbBodies - 1, (nbBodies * (nbBodies - 1)) / 2)
